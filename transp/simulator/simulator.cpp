...
static void matrix_init(void)
{
	...
	array = (int *) xmalloc(N*N * sizeof(int));
	for (int i = 0; i < N; i++)
		for (int j = 0; j < N; j++)
...
static void cache_init(void)
{
	...
	block_to_cache = (cache_node**) xmalloc(memory_blocks * sizeof(struct cache_node *));
	for (int i = 0; i < memory_blocks; i++)
		block_to_cache[i] = NULL;

	lru_head.next = lru_head.prev = &lru_head;
	cache_nodes = (cache_node *) xmalloc(cache_blocks * sizeof(struct cache_node));
